...
class PlayerManager: NSObject, PlayerViewDelegate {
    ...
    private func initPlayerURL() {
        ...
        //视频音频设置
        do {
            _ = try AVAudioSession.sharedInstance().setCategory(.playback, mode: AVAudioSession.Mode.default, options: AVAudioSession.CategoryOptions.mixWithOthers)
        } catch {
            print("音视频设置捕获异常！！！！！！！")
        }
...
extension PlayerManager {
    ...
    func refreshTimeObserve() {
        ...
        if playerView.currentTime >= Int(playerView.endTimeValue) {
            self.moviePlayDidEnd()
        }
        delegate?.playCurrentTime(currentTime: playerView.currentTime)
    }
...
//        }
//        
//        print("Current TIme ---11111  =\(playerView.currentTime)")
    }
}
