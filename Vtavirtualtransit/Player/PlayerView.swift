...
class PlayerView: UIView, UIGestureRecognizerDelegate {
    ...
    public var startTimeValue: Double!
    ...
    fileprivate let PlayFinishColor = RGB_COLOR(12, g: 83, b: 137, alpha: 1) //播放完成颜色
    ...
    private func makeProgress() {
        ...
        progressView.frame = CGRect(x: progressX, y: 0, width: rightTimeLabel.x - progressX , height: Padding)
    ...
    private func makeLeftTimeLab() {
     ...
     //   let totalTimeStr = rightTimeLabel.text ?? initTimeString//取总时长算宽度，避免开始时当前时间字符串长度小于总的
       // let leftTimeWidth = GlobalUtil.textSizeWithString(totalTimeStr, font: leftTimeLabel.font, maxSize: CGSize(width: toolBarView.width, height: toolBarView.height)).width + FIT_SCREEN_WIDTH(5)
        leftTimeLabel.frame =  CGRect(x: 0, y: 0, width: 0, height: Padding)
    ...
    private func makeRightTimeLab() {
      ...
      //  let rightTimeWidth = GlobalUtil.textSizeWithString(totalTimeStr!, font: rightTimeLabel.font, maxSize: CGSize(width: toolBarView.width, height: toolBarView.height)).width + FIT_SCREEN_WIDTH(5)
        rightTimeLabel.frame = CGRect(x: 0, y: 0, width: 0, height: Padding)
    ...
    private func makeMaxButton() {
        ...
        maxButton.frame = CGRect(x: 0, y: 0, width: FIT_SCREEN_WIDTH(24), height: FIT_SCREEN_WIDTH(24))
...
extension PlayerView {
    ...
    func refreshTimeLabelValue(_ time: CMTime) -> String {
        ...
        let endTime = CMTime(seconds: endTimeValue, preferredTimescale: timescale)
        let durationT = endTime.value
        ...
        let totalDurationT = playerItem?.duration.value ?? 0
        let total = Float64(totalDurationT) / Float64(timescale)
        
        progressSlider.minimumValue = Float(startTimeValue / total)
        progressSlider.maximumValue = Float(endTimeValue / total)
...
extension PlayerView {
    ...
    fileprivate func changeVideoProgress(changeType: ProgressChangeType) {
        ...
        print("Progress Value ===?>>. \(progressSlider.value)")
