...
- (void)updateLocationInfo:(CLLocation *)location
                    forKey:(NSString *)key forSnapShot:(FIRDataSnapshot *)snapshot
{
...
- (void)childAdded:(FIRDataSnapshot *)snapshot
{
    @synchronized(self) {
        ...
        if (location != nil) {
            [self updateLocationInfo:location forKey:snapshot.key forSnapShot:snapshot];
        } else {
            // TODO: error?
        }
    }
...
- (void)updateQueries
{
    ...
    [self.locationInfos enumerateKeysAndObjectsUsingBlock:^(id key, GFQueryLocationInfo *info, BOOL *stop) {
        [self updateLocationInfo:info.location forKey:key forSnapShot:nil];
    }];
